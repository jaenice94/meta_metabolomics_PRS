...
for (p in phenotypes) {
	dt <- fread(profiles_vec[[p]]) %>%
		transmute(IID, !!p := PRS_STD) #using scaled PRS
...
fit_baseline <- function(lipid_names, data, covar_terms) {
	...
	do.call(rbind, lapply(lipid_names, function(x) {
		fit <- lm(as.formula(paste(x, "~", rhs)), data = data)
		s <- summary(fit)
		data.frame(model=x, 
		Adj_r_sq_baseline = s$adj.r.squared,
		r_squared_baseline=s$r.squared,
...
fit_prs <- function(prs_col, lipid_names, data, covar_terms){
	...
	do.call(rbind, lapply(lipid_names, function(x) {
		...
		data.frame(model=x,
				...
				SE_PRS = co[prs_col, "Std. Error"],
				T_value_PRS = co[prs_col, "t value"],
				
				N = nobs(fit),
				
				Adj_r_sq_model=s$adj.r.squared,
				r_squared_model=s$r.squared,
...
run_prs_lm <- function(data, lipid_names, prs_map, covars){
	...
	run_one_prs <- function(label, prs_col){
		...
		mutate(adj_r_squared_diff = Adj_r_sq_model - Adj_r_sq_baseline,
			r_squared_diff = r_squared_model - r_squared_baseline, 
				PRS = label)
	}
